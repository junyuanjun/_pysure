...
class Forest():
    ...
    def find_the_min_set(self):
        ...
        # find the most different rule every time,
        ...
        # for rule_ord in range(len(target_set)):
            ...
            r = self.rules[rid]
            r['rid'] = rid
            text_rules.append(self.get_rule_text(r, rid))
        ...
        # get rule fidelity
        arr = [r['r_fidelity'] for r in target_rule_set]

        return {'rules': target_rule_set, 
            "tot_rule": len(self.rules), 
            "correct_coverage": self.coverage,
            "coverage": self.all_coverage,
            "mean_rs_fidelity": float(np.mean(arr)),
            "median_rs_fidelity": float(np.median(arr)),
            "max_rs_fidelity": float(np.max(arr)),
            "min_rs_fidelity": float(np.min(arr)),
            "text_rules": text_rules,
        }

    def get_rule_text(self, r, rid=None):
        rule_str = "If "
        ...
        if self.error_analysis:
            matched_data = self.rule_matched_table_all[rid]
            is_error = self.error_label[matched_data > 0]
            err_rate = len(is_error) / len(matched_data)
            label = f'{np.floor(err_rate*100)}% error ({len(is_error)}/{len(matched_data)})'
        else:
            label = self.target_names[r['label']]
        rule_str += ' Then ' + label
        return rule_str
